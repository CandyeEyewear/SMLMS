...
import { generateQuizPrompt } from '@/lib/openai/prompts/quiz-generation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { moduleTitle, topics, questionCount } = body;

    if (!moduleTitle || !topics || !Array.isArray(topics)) {
      return NextResponse.json(
        { error: 'Module title and topics array are required' },
        { status: 400 }
      );
    }
